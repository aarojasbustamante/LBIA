...
def render_chat_widget(section_name):
    """Render AI chat widget at the end of a section."""
    ...
    for message in st.session_state.chat_history:
        if message["role"] == "user":
            st.markdown(f"""
            <div style='background:#3b82f6;padding:12px 16px;border-radius:12px;margin-bottom:12px;max-width:80%;'>
            ...
            <div style='background:#334155;padding:12px 16px;border-radius:12px;margin-bottom:12px;'>
                <strong style='color:#a78bfa;font-size:11px;'>🤖 AI Assistant</strong>
    ...
    with col_send:
        send_btn = st.button("📤 Send", type="primary", use_container_width=True, key=f"send_{section_name}")
    with col_clear:
        if st.button("🗑️ Clear", use_container_width=True, key=f"clear_{section_name}"):
            st.session_state.chat_history = []
            st.rerun()
    with col_close:
        if st.button("✕ Close", use_container_width=True, key=f"close_{section_name}"):
